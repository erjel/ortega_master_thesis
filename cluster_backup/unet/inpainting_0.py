...
def make_model(input_shape, option=1,num_classes=1,window=0.5):
    """
...
if __name__ == '__main__':
    ...
    window = int(100*windows[int(sys.argv[1])-1])/100
    X_train,Y_train = [],[]
    ...
    callbacks = [tf.keras.callbacks.ModelCheckpoint(
        filepath= f"{path}/11_oct/checkpoints/u_inpainting_{window}_{option}_kernel{kernel_size}",
    ...
    model = make_model(input_shape=image_size + (1,), option=option, num_classes=1,window=window)
    model.compile(
    ...
    np.save(f"{path}/11_oct/history/u_inpainting_{window}_{option}_kernel{kernel_size}.npy",np.array(list(history.history.values())))