def open_frame_sp(frame_num,salt=0.01,pepper=0.01):
    ...
    salty = np.random.choice([True,False],img.shape,p=[salt,1-salt])
    peppery = np.random.choice([True,False],img.shape,p=[pepper,1-pepper])
...
if __name__ == '__main__':
    ...
    for _ in train:
        im = open_frame_sp(_,salt=window,pepper=window)
    ...
    callbacks = [tf.keras.callbacks.ModelCheckpoint(
        filepath= f"{path}/11_oct/checkpoints/u_sp_{window}_{option}_kernel{kernel_size}",
    ...
    np.save(f"{path}/11_oct/history/u_sp_{window}_{option}_kernel{kernel_size}.npy",np.array(list(history.history.values())))