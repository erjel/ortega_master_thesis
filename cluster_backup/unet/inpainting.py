...
def open_frame(frame_num,var=0.5):
...
def get_data_generator(sampler,window):
    def get_data():
        while True:
            yx = next(sampler)
            x,y = yx[1],yx[0]
...
def iteration(input_shape, option=1,num_classes=1,window=0.5):
    """
    ...
    for i in range(int(np.ceil(window*CROP))):
        ...
        if option == 1:
            ...
            coeff = tf.keras.layers.Lambda(lambda z: tf.math.exp(-z),name=f'coeff_{i}')(coeff)
        elif option == 2:
            coeff = tf.keras.layers.multiply([lambdas,norm])
            coeff = tf.keras.layers.Lambda(lambda z: 1./(1. + z),name=f'coeff_{i}')(coeff)
...
if __name__ == '__main__':
    ...
    class Model(tf.keras.Model):    
        ...
        def __init__(self, network, window = 0.5):
            super(Model, self).__init__()
            self.network = network
            self.window = int(np.ceil(window*CROP))
        ...
        def test_step(self, data):
            ...
            for i in range(min(it_lim,int(np.ceil(var*CROP)))):
        ...
        def _compute_loss(self, data,train):
            ...
            #z = z/tf.math.reduce_max(z)


            #return 2 - tf.image.ssim(y,z,max_val = 1.) - tf.math.tanh(tf.image.psnr(y,z,1)/50)
    ...
    windows = np.arange(0.05,0.5,0.05)
    ...
    callbacks = [tf.keras.callbacks.ModelCheckpoint(
        filepath= f"{path}/11_oct/checkpoints/u_inpainting_{var}_{option}_kernel{kernel_size}",
    ...
    denoising = iteration(input_shape=image_size + (1,), num_classes=1,window=window)
    model = Model(denoising,window=window)
    ...
    np.save(f"{path}/11_oct/history/u_inpainting_{var}_{option}_kernel{kernel_size}.npy",np.array(list(history.history.values())))