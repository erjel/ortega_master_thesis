def open_frame_gauss(frame_num,var=1):
    ...
    gauss = np.random.normal(0,var**2,img.size)
    ...
    return input_channel, img_gauss
...
if __name__ == '__main__':
    ...
    callbacks = [tf.keras.callbacks.ModelCheckpoint(
        filepath= f"{path}/11_oct/checkpoints/u_gaussian_{var}_{option}_kernel{kernel_size}",
    ...
    np.save(f"{path}/11_oct/history/u_gaussian_{var}_{option}_kernel{kernel_size}.npy",np.array(list(history.history.values())))