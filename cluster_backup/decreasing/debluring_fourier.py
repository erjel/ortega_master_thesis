...
def fourier(inputs,option,num_classes,typ,order1 = 1):
...
def iteration(input_shape, option=1, num_classes=10,order1 = 1):
    ...
    outputs_r = fourier(inputs_r,option=option,num_classes=num_classes,typ='real',order1 = order1)
    outputs_i = fourier(inputs_i,option=option,num_classes=num_classes,typ='imag',order1 = order1)
...
if __name__ == '__main__':
    ...
    filepath= f"{path}/11_oct/checkpoints/p{order}_deblurring_fourier_{var}_{num_classes}",
    ...
    np.save(f"{path}/11_oct/history/p{order}_debluring_fourier_{var}_{num_classes}.npy",np.array(list(history.history.values())))