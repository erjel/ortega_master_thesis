...
if __name__ == '__main__':
    ...
    class Model(tf.keras.Model):    
        ...
        def __init__(self, network, var = 0.5):
            super(Model, self).__init__()
            self.network = network
            self.window = int(np.ceil(var*CROP))
    ...
    callbacks = [tf.keras.callbacks.ModelCheckpoint(
        filepath= f"{path}/11_oct/checkpoints/p{order}_inpainting_{var}_{num_classes}",
    ...
    np.save(f"{path}/11_oct/history/p{order}_inpainting_{var}_{num_classes}.npy",np.array(list(history.history.values())))