...
def iteration(input_shape, option=1, num_classes=2,window = 0.5):
    ...
    if option == 1:
        ...
        coeff = tf.keras.layers.Lambda(lambda z: tf.math.exp(-z),name=f'coeff')(coeff)
    elif option == 2:
        coeff = tf.keras.layers.multiply([outputs,norm])
        coeff = tf.keras.layers.Lambda(lambda z: 1./(1. + z),name=f'coeff')(coeff)
...
if __name__ == '__main__':
    ...
    callbacks = [tf.keras.callbacks.ModelCheckpoint(
        filepath= f"{path}/11_oct/checkpoints/inpainting_{var}_{option}_kernel{kernel_size}",
    ...
    np.save(f"{path}/11_oct/history/inpainting_{var}_{option}_kernel{kernel_size}.npy",np.array(list(history.history.values())))