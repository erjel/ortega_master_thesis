...
def iteration(input_shape, option=1, num_classes=2,kernel_size=3,pool_size=3):
    ...
    #outputs = tf.keras.layers.Dense(CROP*CROP, activation=activation,name='lambdas')(x)
    #outputs = tf.keras.layers.Reshape((CROP,CROP,1))(outputs)
    outputs = tf.keras.layers.Dense(1, activation=activation,name='lambdas')(x)
...
if __name__ == '__main__':
    ...
    filepath= f"{path}/11_oct/checkpoints/deblurring_cartesian_{var}_{option}_kernel{kernel_size}",
    ...
    np.save(f"{path}/11_oct/history/debluring_cartesian_{var}_{option}_kernel{kernel_size}.npy",np.array(list(history.history.values())))